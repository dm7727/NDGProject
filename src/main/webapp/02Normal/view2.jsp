	...
	<div class="container-fluid" style="background-color: rgb(252, 252, 249);">
		<table class="table table-bordered" border="1" width="90%">
		        ...
		        <td colspan="3" height="100">
		        	...
		        	<c:choose>
		        		<c:when test="${fileType == 'image' }">
		        			<img src="./Uploads/${dto.sfile1 }" style="max-width: 500px" alt="" />
		        		</c:when>
		        		<c:when test="${fileType eq 'music' }">
		        			<audio src="./Uploads/${dto.sfile1 }" controls="controls"></audio>
		        		</c:when>
		        	</c:choose>
		        </td>
		    </tr> 
		    <tr>
		        ...
		        -->
		        <td>            
		           <c:if test="${not empty dto.ofile1 }">
			           ${dto.ofile1 }
			           <a href="./fandownload.do?ofile1=${dto.ofile1 }&sfile1=${dto.sfile1}&idx=${dto.idx}">
			           [다운로드]
			           </a>
		    ...
		    <tr>
		        <td colspan="4" align="center">
		        	<c:choose>
		        		<%-- <c:when test="${dto.userid == userid }"> --%>
		        		<c:when test="${dto.userid == sessionScope.userid }">
		        		<button type="button" onclick="location.href='./fanpass.do?mode=edit&idx=${param.idx}';">
		                수정하기
			            </button>
			            <%-- <button type="button" onclick="location.href='./pass.do?mode=delete&idx=${param.idx}';">
			                삭제하기
			            </button> --%>
			            <button type="button" onclick="location.href='./fanpass.do?mode=delete&idx=${param.idx}';">
			                삭제하기
			            </button>
		        		</c:when>
		        		<%-- <c:otherwise>

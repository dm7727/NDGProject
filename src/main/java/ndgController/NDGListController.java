...
public class NDGListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		*/
		...
		List<ndgboardDTO> boardLists = dao.selectListPage(map);
		//커넥션풀에 자원 반납
		dao.close();
		...
		//View로 포워드를 걸어준다.
		//req.getRequestDispatcher("./02Normal/list.jsp").forward(req, resp);
		...
		if(commandStr.equals("/list.do")){
			req.getRequestDispatcher("/02Normal/list.jsp").forward(req, resp);
		}
		else {
			req.getRequestDispatcher("/01Admin/freeboardlist.jsp").forward(req, resp);
		}
